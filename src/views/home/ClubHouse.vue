      ...
      <template v-else>
        <div class="flex mb-32 items-start flex-col sm:flex-row justify-start sm:justify-between">
          ...
          <div class="btn-area flex flex-row gap-4 justify-between items-center w-full md:w-auto">
            ...
            <router-link
            v-if="user"
              :to="`/pick-location/${user.city.country.id}`"
            ...
            v-else
              ...
              >Book a field</router-link
            >
            <a href="" v-if="display" class="outline-btn w-1/2 md:w-auto justify-center flex">View all</a>
          </div>
        </div>
        <UpcomingBooking @viewAll="displayViewAllButton" class="mb-7" />
...
export default Vue.extend({
  ...
  computed: {
    ...
    country() {
      let value;
      if (this.getPlayer) value = this.getPlayer.city.country.id;
      return value;
    },
    fixtures() {
  ...
  methods: {
    displayViewAllButton(data: boolean) {
      this.display = data;
    },
    async joinTeam() {
      try {
