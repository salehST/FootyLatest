<template>
  <BaseLayout :pageTitle="$t('global.bookings')" :headerPull="'lg'" maxWidth="auto">
  ...
  methods: {
    ...
    async authGoogle() {
      if (this.pageType == 'signup') {
        ...
        let response = await this.socialLogin('google');
        console.log(response, this.error, 'responseresponseresponseresponse');
    ...
    async authFacebook() {
      if (this.pageType == 'signup') {
        ...
        let response = await this.socialLogin('facebook');
        if (response === 'No Player Found') {
          setTimeout(() => {
            // code to be executed after 3 seconds
            this.pageType = 'signup';
    ...
    async socialLogin(socialProvider: string) {
      ...
      console.log('response:', firebaseResponse.user);
      ...
      if (firebaseResponse !== null) {
        ...
        console.log('here after validation', this.error);
        if (!failed) {
          const data = await this.$store.dispatch(
            'appInit',
