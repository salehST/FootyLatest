    ...
    <div v-if="dates" class="bg-white w-full p-4 rounded-md shadow mb-7">
      <div class="inner-info flex items-start flex-col md:flex-row justify-between gap-4 w-full">
        ...
        <div class="text-area w-full">
          <h3
            @click="toggleDiv(1)"
            class="text-brand text-2xl font-700 mb-0 flex w-full items-center justify-between"
          >
            <span class="flex-wrap w-2/3">{{ dates.pitch.location.name }}</span>
          ...
          <div class="booking-more-info overflow-hidden mt-3 md:mt-0" v-show="showDiv === 1">
            <p class="text-brand text-18 md:text-20 font-semibold mb-3">
            ...
            <ul>
              ...
              <li class="flex items-center gap-2 justify-start text-brand text-base">
                ...
                <span
                  >Field {{ dates.pitch.name }} ({{ dates.pitch.type.replace('v', ' vs. ') }})</span
                >
              </li>
            </ul>
  ...
  computed: {
    ...
    dates() {
      ...
      if (upcomingDates.length == 0) {
        this.$emit('viewAll', false);
      } else {
        this.$emit('viewAll', true);
      }
      return upcomingDates.length ? upcomingDates[0] : [];
    },
    activeClass(): string {
