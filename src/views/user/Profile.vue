...
methods: {
  ...
  checkMobile(): void {
    ...
    (this as any).isMobile = mediaQuery.matches;
    if ((this as any).isMobile) {
		(this as any).showDiv = null;
    } else if ((this as any).showDiv === null) {
		(this as any).showDiv = 1 as number | null;
    }
  },
},
mounted() {
  // Recheck the viewport on resize
  window.addEventListener('resize', (this as any).checkMobile);
},
beforeDestroy() {
  // Remove the resize event listener when the component is destroyed
  window.removeEventListener('resize', (this as any).checkMobile);
},
computed: {
